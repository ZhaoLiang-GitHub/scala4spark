...
object ColdStartV9 {
    def main(args: Array[String]): Unit = {
        val Array(lo: String, startDate: String, endDate: String) = args
        var locale = lo
        if (localeInCluster(lo, singapore) && lo != "ID")
            locale = "ID"
        ...
        Array("S").foreach(front7DayLevel => {
            Array("app", "game").foreach(appType => {
                ...
                val allDownload = thisLevel
                    ...
                    .distinct()
                    .count()
                val allCtr = allDownload.toDouble / allExpose.toDouble
                ...
                val tempResult: DataFrame = sparkSession.sparkContext.parallelize(
                    Array(
                        (s"ucbColdStart_${front7DayLevel}_${appType}_V9", app0expose.++(appUCB).toArray.sortBy(_._2).reverse.slice(0, 200).toMap)
                    ))
                    .toDF("id", "apps")
        ...
        rddSaveInHDFS(sparkSession, resultDf.toJSON.rdd, dim_appstore_recommend_tag_doc(endDate, locale2locale(lo), "ucbColdStartV9"))
    }
