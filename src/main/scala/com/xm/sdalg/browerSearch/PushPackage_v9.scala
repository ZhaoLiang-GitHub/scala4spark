...
object PushPackage_v9 {
  def main(args: Array[String]): Unit = {
    ...
    for (hour_of_day <- 8 to 22) {
      ...
      filteredRows.repartition(2).write.mode("overwrite").csv(s"/user/h_data_platform/platform/browser/dwd_browser_push_user_package/packagename=browser-search-v9-${hour_of_day}30/date=${getCurrentDateStr()}")
      sparkSession.sql(
        s"""
           |alter table hive_zjyprc_hadoop.browser.dwd_browser_push_user_package add if not exists partition(packagename='browser-search-v9-${hour_of_day}30',date=${getCurrentDateStr()})
           |""".stripMargin)
    }
